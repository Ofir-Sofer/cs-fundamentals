...
typedef enum Map_Result {
	...
	MAP_UNDERFLOW_ERROR				/**< Underflow error	 		*/
} Map_Result;
 ...
 * @param[in] _capacity - Expected max capacity minimum is 1
 ...
 * @param[in] _map - Hash map to recive pair from, must be initialized
 * @param[in] _key - key to serve as index 
 * @param[in] _pValue - pointer to variable that will get the value if found.
 * @return Success indicator
 * @retval  MAP_SUCCESS	on success
 ...
 * @param[in] _context - User provided context for action function
